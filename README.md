...
```bash
sudo apt install automake bridge-utils curl g++ git imagemagick libglib2.0-bin libglib2.0-dev libssl-dev libtool libwayland-bin libzstd-dev lua-zlib lua-zlib-dev make ncat ninja-build pax-utils pkg-config python3-pip sassc scdoc texinfo unzip wget
pip3 install gi-docgen markdown packaging pygments strenum toml tomli typogrify
```
This project also requires CMake with minimum version 3.25.1.
To get this version on Ubuntu, please follow the steps provided by Kitware
at [https://apt.kitware.com](https://apt.kitware.com/).
