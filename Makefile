...
# vncserv should be enabled by default.  To disable set to 1 on commandline.
CTI_DISABLE_VNCSERV ?= 0

# Make sure QSC_CLT_PATH has been given an value and exists
QSC_CLT_PATH ?= $(shell which qnxsoftwarecenter_clt)
ifeq ("${QSC_CLT_PATH}","")
...
clean:
	...
	-rm -rf $(BUILD)
	-rm -f $(PROJECT_DIR)/install/$(BUILD_NAME).tar.gz
	-rm -f system/desktop.config/weston.ini
...
apk:
	/bin/bash -c "set -a && source $(STAGE_QNX_SDP)/qnxsdp-env.sh && make -Capk TARGET=$(TARGET) QSC_CLT_PATH=$(QSC_CLT_PATH)"
...
weston.ini:
	cat system/desktop.config_common/weston.ini_common $(TARGET_DIR)/weston.ini_$(TARGET) > $(PROJECT_DIR)/system/desktop.config/weston.ini
...
export CTI_DISABLE_VNCSERV=$(CTI_DISABLE_VNCSERV)
endef
