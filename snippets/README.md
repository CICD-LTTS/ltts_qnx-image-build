...
| post_start.~15.networking                               | This snippet contains extra commands, related to networking, to add into the startup script found at `/etc/startup/post_startup.sh`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| post_start.~19.fonts                                    | This snippets inserts font cache updates into the startup flow.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| post_start.~96.multimedia                               | This snippet contains extra commands, that start the multimedia renderer, to add into the startup script found at `/etc/startup/post_startup.sh`. The multimedia renderer is used by other utilities (such as `mmrplay`) for media playback.                                                                                                                                                                                                                                                                                                                                                                                                                 |
| post_start.~97.vncserv                                  | This snippet contains extra commands, that start the VNC server and generate a default password file for VNC on first boot.  This is added to the post startup script found at `/etc/startup/post_startup.sh`.  To disable the VNC server startup you can either remove this snippet or specify CTI_DISABLE_VNCSERV=1 on the command-line to make when building CTI.<br/><br/>The default VNC password is 'qnxuser'.  It is **strongly recommended** to change the VNC password after first boot.  This can be done by running the command 'sudo vncserv -password=\<new-password\>' substituting \<new-password\> with your choice of password.                                                                                                                                                                                           |
| profile.custom                                          | This snippet contains extra environment variables or entries to add into `/etc/profile`. The profile file is sourced by every instance of the shell and hence can be used to add global environment variables to each user's environment when they start a new shell.  A new open source service that is integrated, that would need environment variables to be defined to operate, would require changes to this snippet.                                                                                                                                                                                                                                  |
...
| system_files.custom.sudoers                             | This snippet integrates custom sudoers configurations.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
For example, look for these lines:
    ...
    sensor -U 521:521,1001 -r /data/share/sensor -c /etc/system/config/sensor/rpi4/sensor_demo.conf
    ```
...
and then look for these lines:
    ...
    ```bash
    # Camera Module 3
    #sensor -U 521:521,1001 -b external -r /data/share/sensor -c /etc/system/config/sensor/rpi4/camera_module3.conf
    ```
...
snippet:
    ...
    [Desktop Entry]
    Name=Terminal
    Exec=/usr/bin/st /etc/st_boot.sh
    Icon=/usr/share/icons/qnx_coral_terminal_large.png
    ...
    [Desktop Entry]
    Name=maze
    Exec=/usr/bin/gles2-maze
    Icon=/usr/share/icons/qnx_coral_maze_large.png
    Categories=Utilities
    }
    ...
    [Desktop Entry]
    Name=gears
    Exec=/usr/bin/gles3-gears
    Icon=/usr/share/icons/qnx_coral_gears_large.png
    Categories=Utilities
    }
    ```
...
and change to:
    ...
    Name=Terminal
    Exec=/usr/bin/st /etc/st_boot.sh
    Icon=/usr/share/icons/terminal_black.png
    Categories=Utilities
    }
    ...
    Name=maze
    Exec=/usr/bin/gles2-maze
    Icon=/usr/share/icons/gles2-maze.png
    Categories=Utilities
    }
    ...
    Name=gears
    Exec=/usr/bin/gles3-gears
    Icon=/usr/share/icons/gles2-gears.png
    Categories=Utilities
    }
    ```
...
directory
    ...
    [Desktop Entry]
    Name=cube
    Exec=/usr/bin/vkcubepp
    Icon=/usr/share/icons/qnx_coral_cube_large.png
    Categories=Utilities
    }
    ```
...
and change to:
    ...
    Name=cube
    Exec=/usr/bin/vkcubepp
    Icon=/usr/share/icons/vkcube.png
    Categories=Utilities
    }
    ```
...
Alternatively, you could apply this update instead:
    ...
    Name=Terminal
    Exec=/usr/bin/st /etc/st_boot.sh
    Icon=/usr/share/icons/terminal_white_large.png
    Categories=Utilities
    }
    ...
    Name=maze
    Exec=/usr/bin/gles2-maze
    Icon=/usr/share/icons/google/maze_icon_large_white.png
    Categories=Utilities
    }
    ...
    Name=gears
    Exec=/usr/bin/gles3-gears
    Icon=/usr/share/icons/google/gears_icon_large_white.png
    Categories=Utilities
    }
    ```
...
and
    ...
    Name=cube
    Exec=/usr/bin/vkcubepp
    Icon=/usr/share/icons/google/cube_icon_large_white.png
    Categories=Utilities
    }
    ```
...
Look for this line:
    ...
    ```bash
        demolauncher -a /etc/desktop_files &
    ```

and change it to:

    ```bash
        demolauncher -a /etc/desktop_files -b /etc/images/DSC_0133 &
    ```
